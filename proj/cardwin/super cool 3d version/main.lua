...
function love.load()
	...
	cam = PM.newCamera(1280,720,640,750,-math.pi/2,1200,1.5,0.8)
	
	bg = love.graphics.newImage("resources/bg.png")
...
function love.draw()
	...
	love.graphics.print("offset:           use q/a to change; fov:          use w/s to change",0,30)
	love.graphics.print(cam.o.."",40,30)
	love.graphics.print(cam.f.."",225,30)
...
function love.update(dt)
	world:update(dt)
	mx, my = PM.toWorld(cam,love.mouse.getX(), love.mouse.getY())
	...
	if love.keyboard.isDown("w") then
		cam:setFov(cam.f + 0.01)
	elseif love.keyboard.isDown("s") then
		cam:setFov(cam.f - 0.01)
	end
	if love.keyboard.isDown("q") then
		cam:setOffset(cam.o + 0.001)
	...
	if love.keyboard.isDown("c") then clear = true end
